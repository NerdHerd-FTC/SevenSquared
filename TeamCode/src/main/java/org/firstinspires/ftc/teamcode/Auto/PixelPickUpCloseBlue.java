...
@Config
@Autonomous(name="Close Pixel Pickup - Blue")
public class PixelPickUpCloseBlue extends LinearOpMode {
    DcMotor arm, joint;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory center1 = drive.trajectoryBuilder(startPose)
                ...
                .build();
        // Stafe Right To Get Ready to GO Through Door
        Trajectory center2 = drive.trajectoryBuilder(center1.end())
                .strafeRight(24)
                .build();
        // Turn 180 To Face Door
        TrajectorySequence center2_turn = drive.trajectorySequenceBuilder(center2.end())
                .turn(Math.toRadians(180))
                .build();

        // Spline to pixel stack
        Trajectory center3 = drive.trajectoryBuilder(center2_turn.end())
                .splineTo(new Vector2d(-10, 3), Math.toRadians(180))
                .splineTo(new Vector2d(center_pixel_stack_x, center_pixel_stack_y), Math.toRadians(180))
                .build();
        ...
        TrajectorySequence center1_turn = drive.trajectorySequenceBuilder(center2.end())
                .turn(Math.toRadians(180))
                .build();
        // moving to backdrop
        Trajectory center4 = drive.trajectoryBuilder(center1_turn.end())
                .splineTo(new Vector2d(-16, 10), Math.toRadians(0))
                .splineTo(new Vector2d(20, 13), Math.toRadians(0))
                .splineTo(new Vector2d(center_back_x, center_back_y), Math.toRadians(0))
                .build();

        // move to left corner
        Trajectory cornerCenter = drive.trajectoryBuilder(center4.end())
                .strafeLeft(38)
                .build();
        ...
        TrajectorySequence left1_turn = drive.trajectorySequenceBuilder(left1_2.end())
                .turn(Math.toRadians(180))
                .build();

        Trajectory left2 = drive.trajectoryBuilder(left1_turn.end())
                .splineTo(new Vector2d(-10, 3), Math.toRadians(180))
                .splineTo(new Vector2d(center_pixel_stack_x, center_pixel_stack_y), Math.toRadians(180))
                .build();
        // Raise arm up before executing 3_ series of movements - one issue may be time for later autos
        ...
        TrajectorySequence left2_2_turn = drive.trajectorySequenceBuilder(left2.end())
                .turn(Math.toRadians(180))
                .build();


        Trajectory left3 = drive.trajectoryBuilder(left2_2_turn.end())
                .splineTo(new Vector2d(-16, 10), Math.toRadians(0))
                .splineTo(new Vector2d(20, 13), Math.toRadians(0))
                .splineTo(new Vector2d(left_back_x, left_back_y), Math.toRadians(0))
                .build();
